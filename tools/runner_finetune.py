...
train_transforms_model = transforms.Compose(
    [
         # data_transforms.PointcloudScale(),
         data_transforms.PointcloudRotate(),
         # data_transforms.PointcloudTranslate(),
...
test_transforms_vote = transforms.Compose(
    [
        data_transforms.PointcloudScale(),
        # data_transforms.PointcloudRotate(),
        # data_transforms.PointcloudTranslate(),
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if args.resume:
        ...
        print_log('Load model', logger=logger)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        # print(flops)
        # print(macs)
        # print_log('flops = %.4f' % flops, logger=logger)
        # print_log('macs = %.4f' % macs, logger=logger)
        ...
        # macs, params = profile(base_model, inputs=(input))
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if config.model.NAME == 'PointMamba' or config.model.NAME == 'PointMambaFormer' or config.model.NAME == 'PointMambaFormerPP':
                ...
                cos_sim = F.cosine_similarity(map_global_feature.unsqueeze(1).repeat(1, map_patch_feature.shape[1], 1), map_patch_feature, dim=2)       # 32 * 64

                loss_CE, loss_GLR, loss_importance, acc = base_model.module.get_loss_acc(ret, label, map_patch_feature, map_global_feature, cal_importance, cos_sim)

                loss = args.w_CE * loss_CE + args.w_GLR * loss_GLR + args.w_importance * loss_importance
                _loss = loss
                _loss.backward()
                # forward
                ...
                if train_writer is not None:
                    train_writer.add_scalar('Loss/Batch/Loss_CE', loss_CE.item(), n_itr)
                    train_writer.add_scalar('Loss/Batch/loss_GLR', loss_GLR.item(), n_itr)
                    train_writer.add_scalar('Loss/Batch/loss_importance', loss_importance.item(), n_itr)
            ...
            elif config.model.NAME == 'BasePointMamba':
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            if args.vote:
                if metrics.acc > 92.1 or (better and metrics.acc > 91):
                    metrics_vote = validate_vote(base_model, test_dataloader, 0, val_writer, args, config, logger=logger)
